...
def _extract_full_address(text: str, doc: DocumentText, schedule_text: str = "") -> ExtractionResult:
    ...
    offer_re = _extract_offer_re_premises(doc)
    if offer_re:
        value, page = offer_re
        return make_result(value, 1.0, page, f"Offer Re premises: {value[:80]}",
                           method=ExtractionMethod.rule)
    ...
    numbered_schedule = _extract_henley_numbered_schedule_premises(text)
    if numbered_schedule:
        value = numbered_schedule
        page = next((p.page_num for p in doc.pages if value[:12] in p.text), 0)
        return make_result(value, 1.0, page, f"Numbered schedule premises: {value[:80]}",
                           method=ExtractionMethod.rule)
...
def _extract_building(text: str, doc: DocumentText, premises_value: str | None,
    ...
    if premises_value:
        ...
        if building:
            return make_result(building, 1.0, 0, f"Building from premises: {building}",
                               method=ExtractionMethod.rule)
    return not_found()
...
def _extract_building_from_address(address: str) -> str | None:
    known_as = re.search(
        r"known\s+as\s+([A-Za-z\s]+(?:Tower|Plaza|Centre|Center|Building|House|Mansion))",
        address,
        re.IGNORECASE,
    )
    if known_as:
        return known_as.group(1).strip().title()
...
def _extract_floor_suite(
    ...
    # Offer-letter multi-unit form:
    # "Offices 1101 and 1102 on 11th Floor, LHT Tower..."
    for src in [premises_value or "", text] + [p.text for p in doc.pages[:2]]:
        m = re.search(
            r"\bOffices?\s+(\w+)\s+and\s+(?:Office\s+)?(\w+)\s+on\s+"
            r"(\d{1,2})(?:st|nd|rd|th)?\s+Floor\b",
            src,
            re.IGNORECASE,
        )
        if m:
            suite = f"Offices {m.group(1)} and {m.group(2)}, {m.group(3)}th Floor"
            page = next((pp.page_num for pp in doc.pages if m.group(1) in pp.text and m.group(2) in pp.text), 0)
            return make_result(suite, 1.0, page, suite, method=ExtractionMethod.rule)

    for src in [premises_value or "", text] + [p.text for p in doc.pages[-8:]]:
        m = re.search(r"(Unit\s+\w+,\s+\d{1,2}(?:st|nd|rd|th)\s+Floor)", src, re.IGNORECASE)
        if m:
...
def _extract_offer_re_premises(doc: DocumentText) -> tuple[str, int] | None:
    pattern = re.compile(
        r"Re\s*:\s*Offer\s+to\s+Lease\s*\n\s*(Offices?.+?Hong\s+Kong)\b",
    ...
    for page in doc.pages[:3]:
        ...
        if len(value) > 20:
            return value, page.page_num
    return None


def _extract_henley_numbered_schedule_premises(text: str) -> str | None:
    match = re.search(
        r"THE\s+FOURTH\s+SCHEDULE\s+The\s+said\s+premises\s*:\s*(.+?)(?=THE\s+FIFTH\s+SCHEDULE)",
        text,
        re.IGNORECASE | re.DOTALL,
    )
    if not match:
