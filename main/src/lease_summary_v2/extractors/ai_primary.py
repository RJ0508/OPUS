...
from lease_summary.llm_config import _safe_chat_create
...
def _p_str(x: Any) -> str | None:
    if x is None:
        return None
    s = str(x).strip()
    if s.lower() in {"string", "null", "none", "n/a", "na", "nil"}:
        return None
...
def _is_missing_value(value: Any) -> bool:
    ...
    if isinstance(value, str):
        return value.strip().lower() in {"", "null", "none", "n/a", "na", "nil", "string"}
    return False
...
def _apply(
    ...
    *,
    ...
    for json_key, (path, parser) in _FIELD_MAP.items():
        ...
        if parsed is None:
            continue
        if json_key in _ZERO_IS_MISSING_FIELDS and _is_zero_value(parsed):
            continue
