...
def extract_parties(doc: DocumentText, split: SplitDocument) -> Parties:
    ...
    p.tenant_contact_person = _extract_tenant_contact_person(text, doc)
...
def _extract_landlord_name(text: str, doc: DocumentText, page: int,
    ...
    header_party = _extract_offer_header_party(doc, "Landlord")
    if header_party:
        value, _address, party_page, quote = header_party
        return make_result(value, 1.0, party_page, quote, method=ExtractionMethod.rule)
    ...
    if schedule_text:
        ...
        if value:
            ...
            if len(value) > 5:
                ...
                return make_result(value, 1.0, page, f"Schedule Part IA: {value[:60]}",
                                   method=ExtractionMethod.rule)
...
def _extract_landlord_address(
    ...
    header_party = _extract_offer_header_party(doc, "Landlord")
    if header_party:
        _name, value, party_page, quote = header_party
        return make_result(value, 1.0, party_page, quote, method=ExtractionMethod.rule)
...
def _extract_landlord_agent(doc: DocumentText, page: int, schedule_text: str = "") -> ExtractionResult:
    for p in doc.pages[:5]:
        m = re.search(
            r"([A-Z][A-Z\s]+AGENCY\s+LIMITED)[\s\S]{0,120}?As\s+agent\s+for\s+the\s+Landlord",
            p.text,
        )
        if m:
            value = _normalize_company_name(m.group(1))
            return make_result(value, 1.0, p.page_num, f"Agent for Landlord: {value}",
                               method=ExtractionMethod.rule)
...
def _extract_tenant_contact_person(text: str, doc: DocumentText) -> ExtractionResult:
    result = find_labeled_value(
        text,
        "Tenant’s Primary Contact Person",
        "Tenant's Primary Contact Person",
        "Tenant’s Primary Contact",
        "Tenant's Primary Contact",
    )
    if result:
        label, value = result
        value = re.split(r"\s+(?:Email|Tel|Contact Tel|Business Registration)\s*:", value)[0].strip()
        if 2 < len(value) < 80:
            page = next((p.page_num for p in doc.pages if value[:8] in p.text), 0)
            return make_result(value, 1.0, page, f"{label}: {value}", method=ExtractionMethod.rule)
    m = re.search(
        r"Tenant['’]s\s+Primary\s+Contact\s+Person\s*:\s*([^\n]+)",
        text,
        re.IGNORECASE,
    )
    if m:
        ...
        if 2 < len(value) < 80:
            page = next((p.page_num for p in doc.pages if value[:8] in p.text), 0)
            return make_result(value, 1.0, page, f"Tenant contact: {value}", method=ExtractionMethod.rule)
    return not_found()
...
def _extract_tenant_name(text: str, doc: DocumentText, page: int,
    ...
    header_party = _extract_offer_header_party(doc, "Tenant")
    if header_party:
        value, _address, party_page, quote = header_party
        return make_result(value, 1.0, party_page, quote, method=ExtractionMethod.rule)
    ...
    if schedule_text:
        ...
        if value:
            ...
            if len(value) > 5:
                ...
                return make_result(value, 1.0, page, f"Schedule Part IB: {value[:60]}",
                                   method=ExtractionMethod.rule)
...
def _normalize_address(value: str) -> str:
    ...
    value = re.sub(r"\b(\d+),\s+([A-Za-z])", r"\1 \2", value)
...
def _extract_offer_header_party(
    doc: DocumentText,
    role: str,
) -> tuple[str, str, int, str] | None:
    """Extract From/To header party blocks in offer letters."""
    label = "From" if role.lower() == "tenant" else "To"
    quote_chars = r"\"'\u201c\u201d"
    pattern = re.compile(
        rf"(?m)^\s*{label}\s*:?\s*(?:\n\s*)?(?P<name>[^\n]+)\n"
        rf"(?P<address>.*?Hong\s+Kong)\s*\(\s*the\s+[{quote_chars}]{role}[{quote_chars}]\s*\)",
        re.IGNORECASE | re.DOTALL,
    )
    for page in doc.pages[:3]:
        ...
        if not match:
            continue
        name = _normalize_company_name(match.group("name"))
        address = match.group("address")
        address = re.sub(r"\s*\n\s*", ", ", address).strip(" ,")
        address = re.sub(r",\s*,+", ", ", address)
        address = _normalize_address(address)
        if len(name) > 5 and len(address) > 10:
            quote = f"{label}: {name}; {address}"
            return name, address, page.page_num, quote
    return None
