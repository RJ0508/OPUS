...
def write_excel(
    ...
    _label_write(
        ...
        "E",
        0,
        summary.parties.landlord_name.value or "n/a",
    )
    ...
    _label_write(
        ...
        "E",
        0,
        summary.parties.tenant_name.value or "n/a",
    )
    ...
    if premises_row:
        ...
        # Area in sqft
        if summary.premises.rentable_area_sqft.is_found():
            _write(ws, _resolve_cell(ws, f"G{premises_row}"),
                   float(summary.premises.rentable_area_sqft.value))
        else:
    ...
    if op_ex_row:
        ...
        mgmt_fee = summary.financials.management_fee_monthly_hkd.value
        if mgmt_fee and mgmt_fee != "n/a":
            _write(ws, _resolve_cell(ws, f"F{amount_row}"), float(mgmt_fee))
        else:
            _write(ws, _resolve_cell(ws, f"F{amount_row}"), "n/a")
        mgmt_psf = summary.financials.management_fee_psf_hkd.value
        area = summary.premises.rentable_area_sqft.value
        if mgmt_psf is None and mgmt_fee and mgmt_fee != "n/a" and area and area != "n/a":
            mgmt_psf = Decimal(str(mgmt_fee)) / Decimal(str(area))
        if mgmt_psf is not None and mgmt_psf != "n/a":
            _write(ws, _resolve_cell(ws, f"I{amount_row}"), f"(HK$ {float(mgmt_psf):.2f} per sqft)")
        else:
            _write(ws, _resolve_cell(ws, f"I{amount_row}"), "n/a")
        # Government rent row
        govt_rent = summary.financials.government_rent_monthly_hkd.value
        if govt_rent and govt_rent != "n/a":
            _write(ws, _resolve_cell(ws, f"F{op_ex_row + 3}"), float(govt_rent))
        else:
...
def _render_monthly_rent_section(ws, template_ws, label_map: dict[str, int],
    ...
    rent_amount = summary.financials.monthly_rent_hkd.value
    area = summary.premises.rentable_area_sqft.value
    psf = summary.financials.monthly_rent_psf_hkd.value

    if psf is None and rent_amount and rent_amount != "n/a" and area and area != "n/a":
        psf = Decimal(str(rent_amount)) / Decimal(str(area))

    _write(ws, f"G{rent_row}", "per sq ft")
    if psf is not None and psf != "n/a":
...
def _build_security_deposit_lines(summary: LeaseSummary) -> list[dict[str, Any]]:
    ...
    if components:
        for comp in components[:2]:
            lines.append({"amount": comp.amount, "note": comp.label})
    ...
    elif transferred_amount and transferred_amount != "n/a":
        lines.append({
    ...
    if len(lines) > 3:
        overflow = "; ".join(
            line["note"] for line in lines[2:] if line["note"]
        )
        lines = lines[:2] + [{
            "amount": None,
            "note": overflow or "Additional deposit details",
        }]
    return lines
...
def _render_security_deposit_line(ws, template_ws, row: int, line: dict[str, Any]) -> None:
    ...
    if amount is None:
        ...
        _write(ws, f"F{row}", float(amount))
...
def _format_transferred_deposit_note(summary: LeaseSummary, raw_note: Any) -> str:
    year = None
    if summary.term.lease_commencement_date.is_found():
        year = summary.term.lease_commencement_date.value.year
    if year:
